...
int __cmd_mytrans(const char* filename) {
  ...
  while (fgets(line, sizeof(line), file) != NULL) {
    ...
    while (word != NULL) {
      char lookup_word[256];
      strncpy(lookup_word, word, sizeof(lookup_word) - 1);
      lookup_word[sizeof(lookup_word) - 1] = '\0';
      to_lowercase(lookup_word);

      const char *translation = hash_table_lookup(table, lookup_word);
      printf("原文: %s\t", word);
      if (translation) {
          printf("翻译: %s\n", translation);
      } else {
